...
WhiteTextColor = "#c25c5c"
BlackTextColor = "#494949"
...
class Game:
    ...
    def SelectionLogic(self,index):
        if self.board.isSelecting:
            ...
            TargetPiece = self.board.GetCell(self.TargetIndex).piece
            if self.turn%2 == 0:
                CanMove = TargetPiece.color != "WHITE"
            else:      
                CanMove = TargetPiece.color != "BLACK"
            if CanMove: 
                CapturedPiece = self.board.MovePieces(self.SlectedIndex,self.TargetIndex)
                Turn += 1
            if CapturedPiece != None:
                if self.turn%2 == 0:
                    self.whiteCaptures.append(CapturedPiece.rank)
                else:
                    self.blackCaptures.append(CapturedPiece.rank)
            self.SlectedIndex, self.TargetIndex = None, None
        else:
            if self.turn%2 == 0 and self.board.GetCell(index).piece.color == "WHITE":
                self.SlectedIndex = index
                self.board.SelectPos(index)
            elif self.turn%2 == 1 and self.board.GetCell(index).piece.color == "BLACK":
                self.SlectedIndex = index
                self.board.SelectPos(index)
    def run(self):
        self.running = True
...
class Piece:
    ...
    def drawPiece(self,pos):
        PieceText = self.LettrePieceDict.get(self.rank,'')
        PieceColor = self.ColorPieceDict.get(self.color,'#ffffff')
        RenderedText = Textfont.render(PieceText,True,PieceColor)
...
pygame.font.init()
Textfont = pygame.font.SysFont('Arial', 30, bold=False)
